package com.arbbot.exchange.htx;

import com.arbbot.exchange.MultiConnectionWsClient;
import com.arbbot.health.HealthMonitor;
import com.arbbot.market.OrderBookManager;
import okhttp3.OkHttpClient;
import java.util.List;
import java.util.stream.Collectors;

public class HtxWsClient extends MultiConnectionWsClient {

    static final int SYMBOLS_PER_SHARD = 45;

    public HtxWsClient(String wsBaseUrl, List<String> symbols,
                       OrderBookManager manager, HealthMonitor healthMonitor,
                       OkHttpClient httpClient) {
        super("htx",
            partition(symbols, SYMBOLS_PER_SHARD).stream()
                .map(slice -> new HtxWsClientShard(wsBaseUrl, slice, manager, healthMonitor, httpClient))
                .collect(Collectors.toList()));
    }
}
