...
class BinanceIntegrationTest {
    ...
    void allBooksInitializeAndReceiveUpdates() throws InterruptedException {
        // 3 symbols — well within 190/shard limit, so 1 shard
        ...
        try {
            ...
            while (System.currentTimeMillis() < deadline) {
                boolean allInit = symbols.stream()
                    .allMatch(sym -> obm.getOrCreateBook("binance", sym).isInitialized());
                if (allInit) break;
                Thread.sleep(500);
            }

            assertTrue(
                symbols.stream().allMatch(sym -> obm.getOrCreateBook("binance", sym).isInitialized()),
                "All books must be initialized within 60s");

            Map<String, Instant> lastUpdateBefore = new HashMap<>();
            for (String sym : symbols)
                lastUpdateBefore.put(sym, obm.getOrCreateBook("binance", sym).getLastUpdateTime());

            Thread.sleep(120_000);

            for (String sym : symbols) {
                Instant before = lastUpdateBefore.get(sym);
                Instant after = obm.getOrCreateBook("binance", sym).getLastUpdateTime();
